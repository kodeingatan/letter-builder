...
/**
 * Task 21 — Designer-gated reorder/icon update for a Persuratan menu entry.
 * Covered by the existing `Administration Management` permission grant.
 */
export default defineEventHandler(async (event) => {
  await requireApiAccess(event)
  const id = Number(getRouterParam(event, 'id'))
  if (!Number.isInteger(id) || id < 1) throw createError({ statusCode: 400, message: 'Invalid administration id' })
  ...
  try {
    return await NavigationService.updateAdministrationMenu(id, parsed.data)
  } catch (e: any) {
