...
import { TableDataExportQuerySchema } from '~~/server/dto/table-data.dto'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = TableDataExportQuerySchema.safeParse(getQuery(event))
  if (!parsed.success) throw createError({ statusCode: 400, message: parsed.error.errors[0].message })

  try {
    const { filename, content } = await TableDataService.exportCsv(tableName, parsed.data)
