...
import { PreviewComponentSchema } from '~~/server/dto/components.dto'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = PreviewComponentSchema.safeParse(body)
  if (!parsed.success) throw createError({ statusCode: 422, message: parsed.error.errors[0].message })
  try {
    return await ComponentsService.preview(id, parsed.data)
  } catch (e: any) {
