...
import { ValidateTreeSchema } from '~~/server/dto/templates.dto'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = ValidateTreeSchema.safeParse(body)
  if (!parsed.success) throw createError({ statusCode: 422, message: parsed.error.errors[0].message })
  try {
    return await TemplatesService.validateTree(id, parsed.data.content)
  } catch (e: any) {
