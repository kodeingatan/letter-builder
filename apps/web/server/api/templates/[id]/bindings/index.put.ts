...
import { BulkBindingsSchema } from '~~/server/dto/template-bindings.dto'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = BulkBindingsSchema.safeParse(body)
  if (!parsed.success) throw createError({ statusCode: 422, message: parsed.error.errors[0].message })
  try {
    return await TemplateBindingsService.bulkUpsert(id, parsed.data)
  } catch (e: any) {
