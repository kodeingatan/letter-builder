...
import { ref } from 'vue'
import type { Guard, QueryGuard } from '~/shared/types/guard'
...
export const useGuardsStore = defineStore('guards', () => {
  ...
  async function fetchAll(params?: Partial<QueryGuard>) {
    ...
    try {
      ...
      error.value = e.data?.message || 'Failed to fetch guards'
      throw e
    } finally {
      loading.value = false
    }
  ...
  async function create(data: Partial<Guard>) {
    loading.value = true
    error.value = null
    try {
      const response = await $fetch<Guard>('/api/guards', {
        method: 'POST',
        body: data,
        headers: { Authorization: `Bearer ${useAuthStore().token}` },
      })
      ...
      error.value = e.data?.message || 'Failed to create guard'
      throw e
    } finally {
      loading.value = false
    }
  ...
  async function update(id: number, data: Partial<Guard>) {
    loading.value = true
    error.value = null
    try {
      const response = await $fetch<Guard>(`/api/guards/${id}`, {
        method: 'PUT',
        body: data,
        headers: { Authorization: `Bearer ${useAuthStore().token}` },
      })
      ...
      error.value = e.data?.message || 'Failed to update guard'
      throw e
    } finally {
      loading.value = false
    }
  ...
  async function remove(id: number) {
    ...
    try {
      ...
      error.value = e.data?.message || 'Failed to delete guard'
      throw e
    } finally {
      loading.value = false
    }
